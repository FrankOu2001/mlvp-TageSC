...
class Tage:
    def __init__(self, lfsr):
        ...
        self.tn = tuple(TaggedPredictor() for _ in range(4))
        self.use_alt_on_na_ctrs = tuple((UACtr(), UACtr()) for _ in range(128))
        self.bank_tick_ctrs = (BTCtr(), BTCtr())
        self.lfsr = lfsr

    def gets(self, pc: int, folded_histories: tuple[tuple[FoldedHistory, ...], ...]) -> tuple[bool, bool]:
        res = [*self.t0.gets(pc)]
        for way in range(2):
    ...
    def _alloc(self, idx: int, tag: int, provider: int, train_taken: bool, way: int):
        if provider == 4:
