...
def is_hit_no_table(way: int):
    def hit_no_table(dut: DUTTage_SC) -> bool:
        provided = getattr(dut, f"Tage_SC_s2_provideds_{way}").value
        return dut.io_s1_ready.value == 1 and dut.io_s2_fire_1.value == 1 and provided == 0
...
def is_hit_table_i_weak(way: int, t_i: int, use_alt: int):
    def hit_table_i_weak(dut: DUTTage_SC) -> bool:
        ...
        return (dut.io_s1_ready.value and dut.io_s2_fire_1.value and provided and provider == t_i
                and unconfident == 1 and alt_used == use_alt)
...
def get_coverage_group_of_tage_predict(dut: DUTTage_SC) -> CovGroup:
    ...
    """Tage Predict function coverage begin"""
    for w in range(2):
        # 第w个指令槽历史表ti命中
        for i in range(4):
    ...
    for use_alt in range(2):
        ...
        group.add_watch_point(
            dut,
            {
                f"all_slots_hit_same_table_weak {alt_use_str[use_alt]}": lambda dut:
                (is_hit_table_i_weak(0, 0, use_alt)(dut) and is_hit_table_i_weak(1, 0, use_alt)(dut)) or
                (is_hit_table_i_weak(0, 1, use_alt)(dut) and is_hit_table_i_weak(1, 1, use_alt)(dut)) or
                (is_hit_table_i_weak(0, 2, use_alt)(dut) and is_hit_table_i_weak(1, 2, use_alt)(dut)) or
                (is_hit_table_i_weak(0, 3, use_alt)(dut) and is_hit_table_i_weak(1, 3, use_alt)(dut))
            },
            name=point_name
        )
    """Tage Predict function coverage end"""
